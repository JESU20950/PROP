...
import static Domain.Problem.introduce_problem_toBD;

public class ModificateFEN extends JPanel {
    private FrameProgram frame;
    private EliminateFEN eliminate;

    ModificateFEN(FrameProgram t){
        frame = t;
        this.setLayout(new GridLayout());
        eliminate = new EliminateFEN(frame);
        eliminate.getGraphic_list().removeMouseListener(eliminate.getGraphic_list().getMouseListeners()[0]);
        eliminate.getGraphic_list().addMouseListener(new clear());
        this.add(eliminate);
    }
    private class clear extends MouseAdapter {
        ...
        public void mouseClicked(MouseEvent me) {
            if (me.getClickCount() == 2) {
                oldFEN = eliminate.getGraphic_list().getSelectedItem();
                ...
                eliminate.remove(eliminate.getTable());
                eliminate.setTable(new SampleGame(eliminate.getGraphic_list().getSelectedItem()));
                eliminate.add(eliminate.getTable(),BorderLayout.SOUTH);
                frame.getMiFrame().revalidate();
                frame.getMiFrame().repaint();
            }
        }
        private class IntroduceFEN extends JPanel {
            private FrameProgram frame;
            ...
            private class double_click extends MouseAdapter {
                public void mouseClicked(MouseEvent me) {
                    if (me.getClickCount() == 1) {
                        ...
                        try {
                            number_of_plays_int = number_of_plays_int/2;
                            modificate_problem_fromBD(oldFEN,convertParameterstoFEN(FENtext, player_who_start_bool, player_who_has_to_win, number_of_plays_int));
